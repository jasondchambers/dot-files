...
local function create_autocmds()
  -- Auto commands are like event listeners
...
local function set_colorscheme()
  ...
  -- Defer background to terminal settings e.g. if the terminal is transparent
  -- make Neovim transparent too
  vim.api.nvim_set_hl(0, "Normal", { bg = "none" })
  vim.api.nvim_set_hl(0, "NormalFloat", { bg = "none" })
end

local function set_greeter()
  require('jasonchambers.alpha-custom-greeter')
end
...
local function configure_plugins(plugins)
  ...
  plugins.telescope.setup({
    defaults = {
      ...
      mappings = {
        i = {
          ['<C-k>'] = plugins.telescope_actions.move_selection_previous,
          ['<C-j>'] = plugins.telescope_actions.move_selection_next,
          ['<C-q>'] = plugins.telescope_actions.send_to_qflist + plugins.telescope_actions.open_qflist,
        },
      },
    },
  })
  ...
  -- Treesitter section
  plugins.nvim_treesitter.install({
    'bash',
    'python',
    'javascript',
    'typescript',
  ...
  plugins.render_markdown.setup({})
end

local function set_keymaps(plugins)
  ...
  vim.keymap.set('n', ']h', plugins.gitsigns.next_hunk, { desc = 'Next git hunk' })                                       -- GitSigns
  vim.keymap.set('n', '[h', plugins.gitsigns.prev_hunk, { desc = 'Previous git hunk' })                                   -- GitSigns
  vim.keymap.set('n', '<leader>gp', plugins.gitsigns.preview_hunk, { desc = 'Preview hunk' })                             -- GitSigns
  vim.keymap.set('n', '<leader>gs', plugins.gitsigns.stage_hunk, { desc = 'Stage hunk' })                                 -- GitSigns
  vim.keymap.set('n', '<leader>gr', plugins.gitsigns.reset_hunk, { desc = 'Reset hunk' })                                 -- GitSigns
  vim.keymap.set('n', '<leader>gb', plugins.gitsigns.blame_line, { desc = 'Blame line' })                                 -- GitSigns
  ...
  vim.keymap.set('n', '<C-h>', plugins.smart_splits.move_cursor_left)                                                     -- smart-splits
  vim.keymap.set('n', '<C-j>', plugins.smart_splits.move_cursor_down)                                                     -- smart-splits
  vim.keymap.set('n', '<C-k>', plugins.smart_splits.move_cursor_up)                                                       -- smart-splits
  vim.keymap.set('n', '<C-l>', plugins.smart_splits.move_cursor_right)                                                    -- smart-splits
